...
if (localStorage.getItem("load")) {
    //LocalStorage varsa gir
    ArrayTask = JSON.parse(localStorage.getItem("load"));   //LocalStorage çağır
    ArrayTask.forEach(function (
        element                                          //LocalStorage 'deki bilgiyi aktarmak için fonksiyon
    ) {
        i++;
        ...
        liDOM.innerHTML =
            //liste elemanına Eklencek bilgiyi atama
            `
        ${element.task} 
        <button
        class="close" 
        ...
        if (ArrayTask[i - 1].check) {
            //liste elemanı "check" edilmiş ise işaretleme
...
function formHandler(event) {
    ...
    if (TASK.value.trim() == "") {
        //"input" değeri boş ise "toast" bildirimi ile
...
function RemoveFunc(j) {
    ...
    let index = ArrayTask.findIndex(function (Atask) {
        //silinecek liste elemnını LocalStorage içindeki "index"ine ulaşmak
...
document.addEventListener(
    "click",
    (
        element //tıklanan liste elemanına ulaşma
    ) => {
        if (element.target.matches("li")) {
            //liste elemanı mı diye kontrol
            let elementId = element.target.id; //liste elemanının "id" sine ulaşma
            let index = ArrayTask.findIndex(function (Atask) {
                //liste elemanının LocalStorage içindeki "index"ine ulaşmak
